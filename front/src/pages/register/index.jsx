...
export default function Register() {
  ...
  const validar = () => {
    if (!form.username || !form.nome || !form.tipo || !form.password || !form.password2) {
  ...
  const cadastrar = async (e) => {
    ...
    try {
      await axios.post("http://127.0.0.1:8000/api/register/", {
        ...
        email: form.email,      // seu model tem email
        nome: form.nome,
        telefone: form.telefone,
        tipo: form.tipo,
        password: form.password,
        password2: form.password2, // se seu backend não usar, remova
      });
